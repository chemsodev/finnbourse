...
export default function TCCUsersPage({ params }: TCCUsersPageProps) {
  ...
  useEffect(() => {
    if (hasRestToken && !tokenLoading) {
      console.log("🔄 Triggering TCC data load...");
      loadTCCData();
    }
  }, [hasRestToken, tokenLoading]);

  // Debug logging for TCC data
  useEffect(() => {
    console.log("🔍 TCC Data Debug:", {
      tcc,
      hasUsers: !!tcc?.users,
      usersLength: tcc?.users?.length,
      users: tcc?.users,
      isLoading,
      tokenLoading,
      hasRestToken,
    });
  }, [tcc, isLoading, tokenLoading, hasRestToken]);

  // Force data reload on component mount
  useEffect(() => {
    const forceReload = async () => {
      if (hasRestToken && !isLoading && !tcc) {
        console.log("🔄 Force reloading TCC data...");
        await loadTCCData();
      }
    };

    // Add a small delay to ensure token is properly set
    const timer = setTimeout(forceReload, 1000);
  ...
  const hasUsers =
    tcc?.users && Array.isArray(tcc.users) && tcc.users.length > 0;
          ...
          {!hasUsers ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={loadTCCData} variant="secondary" size="sm">
                  Reload Data
                </Button>
              </div>
              <div className="mt-4 p-4 bg-gray-50 rounded text-sm">
                ...
                <br />
                TCC: {tcc ? "Present" : "Missing"}
                <br />
                Users: {tcc?.users ? `Array(${tcc.users.length})` : "Missing"}
                <br />
                Loading: {isLoading ? "Yes" : "No"}
                <br />
                Users Type: {typeof tcc?.users}
                <br />
                Is Array: {Array.isArray(tcc?.users) ? "Yes" : "No"}
                <br />
                Raw TCC: {JSON.stringify(tcc, null, 2)}
              </div>
            </div>
          ) : (
