  ...
  return (
    <div className="w-full h-full min-h-[400px] flex flex-col flex-1">
      <div className="flex items-center mb-2">
        <h2 className="text-xl font-bold text-gray-900">{t("titre")}</h2>
      </div>
      {/* Switch Buttons */}
      <div className="flex items-center justify-between mb-4 w-full">
        <div className="inline-flex shadow border border-gray-200">
          <button
            className={`px-4 py-1 text-[0.8vw] font-medium focus:z-10 transition-colors transition-bg duration-200 ${
              view === "M.primaire"
          ...
          <TableBody className="bg-gray-50 h-full">
            {displayedTitles.map((item) => (
              <TableRow key={item.id} className="bg-gray-50">
                ...
                <TableCell className="font-medium text-gray-900 max-w-[220px] truncate bg-gray-50">
                  {item.titre}
                </TableCell>
                <TableCell className="text-gray-700 bg-gray-50">
                  {item.type}
                </TableCell>
                <TableCell className="text-gray-500 max-w-[300px] truncate bg-gray-50">
                  {item.Quantité}
                </TableCell>
